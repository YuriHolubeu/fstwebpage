<template>
  <div class="project-page q-pa-md q-pb-xl">
    ...
    <section
      v-if="showFeaturedProductPreview"
      ref="reasonsWithoutAiEl"
      class="screenshot-showcase q-pt-xl q-mt-lg"
    >
      <div
        class="row q-col-gutter-xl items-center workspace-row reveal-on-scroll"
        :class="{ 'is-visible': reasonsWithoutAiVisible }"
      >
        <div class="col-12 workspace-copy">
          <h2 class="screenshot-title text-h4 text-weight-bold pp-heading q-mb-sm">
            Reasons why the application is useful (even without AI tools)
          </h2>

          <ul class="workspace-features q-pl-none q-ma-none">
            <li
              v-for="reason in reasonsWithoutAiFeatures"
              :key="reason.text"
              class="workspace-feature row no-wrap items-start"
            >
              <q-icon :name="reason.icon" size="22px" class="workspace-feature__icon q-mr-md" />
              <p class="workspace-feature__text text-caption pp-muted q-mb-none">
                {{ reason.text }}
              </p>
            </li>
      ...
      >
        ...
        <div v-else class="row q-col-gutter-lg application-content-row">
          ...
          <div class="col-12 col-md-6 application-screenshot-col">
            ...
            <p class="application-screenshot-caption q-mb-none">
              A PDF viewer, a LaTeX editor, and a file explorer in one application
            </p>
          </div>
        </div>
      </div>
    ...
    >
      ...
      <article
        v-if="showFeaturedProductPreview"
        class="tool-shot-feature reveal-on-scroll"
        :class="{ 'is-visible': keyToolsVisible }"
      >
        <button
          type="button"
          class="application-screenshot-trigger tool-shot-feature__trigger"
          :aria-label="`Open ${featuredProductPreview.title} screenshot`"
          @click="openToolPreview(featuredProductPreview)"
        >
          <figure class="tool-shot-feature__figure">
            <img
              :src="featuredProductPreview.src"
              :alt="featuredProductPreview.title"
              class="tool-shot-feature__img"
              :width="featuredProductPreview.width"
              :height="featuredProductPreview.height"
              loading="lazy"
              decoding="async"
            />
          </figure>
        </button>
        <ul class="tool-shot-feature__highlights workspace-features q-pl-none q-ma-none">
          <li
            v-for="highlight in featuredProductPreviewHighlights"
            :key="highlight"
            class="workspace-feature row no-wrap items-start"
          >
            <q-icon name="check_circle" size="20px" class="workspace-feature__icon tool-shot-feature__highlight-icon" />
            <p class="workspace-feature__text text-caption pp-muted q-mb-none">
              {{ highlight }}
            </p>
          </li>
      ...
      <div class="tool-shot-grid row q-col-gutter-xl justify-center full-width">
        ...
        >
          ...
          >
            <button
              type="button"
              class="application-screenshot-trigger tool-shot-card__trigger"
              :aria-label="`Open ${item.title} screenshot`"
              @click="openToolPreview(item)"
            >
              <figure class="application-screenshot-figure tool-shot-card__figure">
                <img
                  :src="item.src"
                  :alt="item.title"
                  class="application-screenshot-img tool-shot-card__img"
                  :width="item.width"
                  :height="item.height"
                  loading="lazy"
                  decoding="async"
...
const props = defineProps({
  showFeaturedProductPreview: {
    type: Boolean,
    default: false
  }
})

const graphPreviewSrc = `${import.meta.env.BASE_URL}graph%20preview.png`
...
const reasonsWithoutAiFeatures = [
  {
    icon: 'psychology',
    text: 'It gives more space for thoughts'
  },
  {
...
const featuredProductPreviewHighlights = [
  ...
  'A convenient way to display and navigate information',
  'An integrated LaTeX editor for direct editing'
]
...
onMounted(() => {
  ...
  reasonsWithoutAiObserver = new IntersectionObserver(
    (entries) => {
      for (const e of entries) {
        if (e.isIntersecting) {
          reasonsWithoutAiVisible.value = true
        }
      }
    },
    { threshold: 0.12, rootMargin: '0px 0px -32px 0px' }
  )
  if (props.showFeaturedProductPreview && reasonsWithoutAiEl.value) {
    reasonsWithoutAiObserver.observe(reasonsWithoutAiEl.value)
  }
...
.application-screenshot-caption {
  margin: 10px 0 0;
  padding: 0 0.35rem;
  max-width: 100%;
  font-size: 1.05rem;
...
.gallery-block--tools {
  --tools-preview-width: min(1200px, calc(100vw - 32px));
  width: var(--tools-preview-width);
  max-width: var(--tools-preview-width);
  margin-left: calc(50% - var(--tools-preview-width) / 2);
  margin-right: calc(50% - var(--tools-preview-width) / 2);
}
...
.tool-shot-feature__trigger:focus-visible {
  outline: 2px solid rgba(94, 234, 212, 0.9);
  outline-offset: 3px;
}

.tool-shot-feature__figure {
  width: 100%;
  aspect-ratio: 1917 / 1132;
  display: flex;
  align-items: center;
  justify-content: center;
  margin: 0;
...
.tool-shot-feature__trigger:hover .tool-shot-feature__figure {
  border-color: rgba(255, 255, 255, 0.22);
  box-shadow:
    0 0 0 1px rgba(3, 117, 204, 0.2),
    0 16px 48px rgba(6, 15, 20, 0.75);
...
.tool-shot-feature__highlights .workspace-feature {
  column-gap: 0.75rem;
}

.tool-shot-feature__highlight-icon {
  flex: 0 0 20px;
  width: 20px;
  margin-right: 0 !important;
}

.tool-shot-feature__highlights .workspace-feature + .workspace-feature {
  margin-top: 0.65rem;
}
...
.tool-shot-card__trigger:hover .tool-shot-card__figure {
  border-color: rgba(255, 255, 255, 0.22);
  box-shadow:
    0 0 0 1px rgba(3, 117, 204, 0.2),
    0 16px 48px rgba(6, 15, 20, 0.75);
